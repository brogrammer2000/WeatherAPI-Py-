...
def APIcall(): #putting the coode in one method so that we can pass it in scheduler
 ...
 for city in cities: #using for loop to loop through the cities
  ...
  with open("data123.txt", "a") as cleaned: #opening the local file to store the cleaned data
  ...
  with open("data1234.txt", "a") as raw: #opening the local file to store the raw data
